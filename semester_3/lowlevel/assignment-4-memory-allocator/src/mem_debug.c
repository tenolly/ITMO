...
void debug_struct_info(FILE *f, void const *addr) {
    ...
    for (size_t i = 0; i < DEBUG_FIRST_BYTES && i < header->capacity.bytes; ++i) fprintf(f, "%hhX", header->contents[i]);
...
void debug_heap(FILE *f, void const *ptr) {
    ...
    for (struct block_header const *header = ptr; header; header = header->next) debug_struct_info(f, header);
}
